...
class Thermostat
  ...
  def get_desired_temperature(target_temp)
    puts "Hello, the target_temp is #{target_temp}"
  ...
  def self.detect_temperature(all_temps, target_temp)
    all_temps.each do |temp_reading|
      temp = Thermostat.new(temp_reading) #use our instance factory to create an instance  of class Thermostat
      binding.pry
      temp.get_desired_temperature(target_temp) #call that awesome instance method we created
    end
  end
